...
var feststages = stagesmodes.stages_splatfest;
...
var stageConversionCodes = {"xx": "Shifty Station", "Shifty Station": "xx"};
...
function makeShiftyTable(list) {
	var listnotempty = false;
	if (list && list.length > 0) { listnotempty = true; }
	var txt = "<table id='shiftyTable'><thead><tr><td>Shifty Station</td><td></td></tr></thead><tbody>";
	for (s = 0; s < feststages.length; s++) {
		var matched = false;
		if (listnotempty) {
			for (i = 0; i < list.length; i++) {
		...
		//txt += "<tr class='selected' data-stage='" + feststages[s].id + "'><td>" + feststages[s].id + "</td><td>" + feststages[s].name + "</td><td class='checkcross'></td></tr>";
	}
	txt += "</tbody></table>";
	
	$("div#shiftySettingsTable").html(txt);
	shiftyCount();
}
...
function getStageList() {
	...
	$(".stageselect.selected").each(function(i) {
		arr.push({mode: convertStageCode($(this).attr("data-mode")), stage: convertStageCode($(this).attr("data-stage")), shifty: false});
	});
	return arr;
}
...
function getShiftyList() {
	var arr = [];
	var i = 0;
	$("#shiftyTable tr.selected").each(function(i) {
		//arr.push($(this).children("td").eq(1).text());
		arr.push({mode: "Turf War", stage: $(this).children("td").eq(0).text(), shifty: true});
	});
	return arr;
}
...
function shiftyCount() {
	var c = $("#shiftyTable tr.selected").length;
	$("#shiftycounts").html("Selected Shifty Stations: " + c);
	return c;
}
...
function stageBox(stage) {
	if ($("input#chk-textonly").is(':checked')) {
		...
		return "<div class='stage fancy clickable'><span class='stagepic " + stage.stage.toLowerCase().replace(/[^a-z]/g, "") + "'><span class='uncheckstage'>Uncheck this<br />stage+mode</span></span><span class='modename'>" + (stage.shifty === true ? "Shifty Station" : stage.mode) + "</span><br /><span class='stagename'>" + stage.stage.replace(/'/g, "&#39;") + "</span></div>";
	}
}
...
function weaponBox(wpn, player) {
	if ($("input#chk-textonly").is(':checked')) {
		...
		if ($("input#chk-showsubspecial").is(':checked')) {
			...
			txt = txt + "<div class='SubSpcContainer'><span class='weaponsub'>" + wpn.sub + "</span><br /><span class='weaponspecial" + ((wpn.special.toLowerCase().slice(-13) == "bomb launcher") ? " twoline" : "") + "'>" + wpn.special + "</span></div>";
		}
...
$(function()
{
	$(document.body).on("click", ".showhidesettings, .showhideshifty, .showhideweapons", function() {
	...
	$(document.body).on("click", "button#btn-stringtostages", function() {
		...
		$("#shiftyTable tbody tr").removeClass("selected");
		for (i = 0; i < arr2.length; i++) {
			$("#shiftyTable tbody tr[data-stage='" + arr2[i] + "'").addClass("selected");
		}
		shiftyCount();
	...
	$(document.body).on("click", "#shiftyTable tbody tr", function() {
	...
	$(document.body).on("click", ".shiftyselectall", function() {
		$("#shiftyTable tbody tr").addClass("selected");
		if ($("input#chk-savesettings").is(':checked')) { saveLoadSettings("save"); }
		shiftyCount();
	});
	$(document.body).on("click", ".shiftyselectnone", function() {
		$("#shiftyTable tbody tr").removeClass("selected");
		if ($("input#chk-savesettings").is(':checked')) { saveLoadSettings("save"); }
		shiftyCount();
	});
	...
	$(document.body).on("click", ".stage.clickable", function() {
		...
		if (m === "xx") {
			$("#shiftyTable tbody tr[data-stage='" + s + "'").removeClass("selected");
			shiftyCount();
		} else {
	...
	$("div#updated").html("Last updated (dd/mm/yyyy):<br />Randomizer script: 31/07/2019<br />Stage list: " + stagesmodes.updated.date + ((stagesmodes.updated.note.length > 0) ? " (" + stagesmodes.updated.note.trim() + ")" : "") + "<br />Weapon list: " + weaponlist.updated.date + ((weaponlist.updated.note.length > 0) ? " (" + weaponlist.updated.note.trim() + ")" : ""));
});
...
function saveSettings() {
	...
	localStorage.splatoon2_elim_settings = JSON.stringify(settings);
}

function loadSettings() {
	if (localStorage.splatoon2_elim_settings === undefined) return;
	if (localStorage.splatoon2_elim_settings === "disabled") {
		document.getElementById("chk-savesettings").checked = false;
		return;
	}
	const savedSettings = JSON.parse(localStorage.splatoon2_elim_settings);
...
function saveWeapons() {
	...
	localStorage.splatoon2_elim_weapons = JSON.stringify({"draft": draftstatus, "weapons": weaponstates});
}

function loadWeapons() {
	if (localStorage.splatoon2_elim_weapons === undefined) return true;
	if (localStorage.splatoon2_elim_weapons === "disabled") {
		document.getElementById("chk-saveweapons").checked = false;
		return true;
	}
	
	const savedWeapons = JSON.parse(localStorage.splatoon2_elim_weapons);
	const newweapons = [];
...
document.addEventListener("DOMContentLoaded", function() {
	...
	document.getElementById("chk-savesettings").addEventListener("change", function() {
		if (this.checked === false) {
			localStorage.splatoon2_elim_settings = "disabled";
			return;
		}
		saveSettings();
	});
	document.getElementById("chk-saveweapons").addEventListener("change", function() {
		if (this.checked === false) {
			localStorage.splatoon2_elim_weapons = "disabled";
			return;
		}
		saveWeapons();
	});

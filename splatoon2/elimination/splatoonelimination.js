...
function startingFunc() {
	const multiban = [ //every array should contain scoped and unscoped chargers with the same kit (and the hero skin for regular splat charger)
		...
		["Firefin Splat Charger", "Firefin Splatterscope"],
		["Kensa Charger", "Kensa Splatterscope"],
...
function saveSettings() {
	...
	localStorage.setItem("splatoon2_elim_settings", JSON.stringify(settings));
}

function loadSettings() {
	if (localStorage.getItem("splatoon2_elim_settings") === null) return;
	if (localStorage.getItem("splatoon2_elim_settings") === "disabled") {
		document.getElementById("chk-savesettings").checked = false;
		return;
	}
	const savedSettings = JSON.parse(localStorage.getItem("splatoon2_elim_settings"));
...
function saveWeapons() {
	...
	localStorage.setItem("splatoon2_elim_weapons", JSON.stringify({"draft": draftstatus, "weapons": weaponstates}));
}

function loadWeapons() {
	if (localStorage.getItem("splatoon2_elim_weapons") === null) return true;
	if (localStorage.getItem("splatoon2_elim_weapons") === "disabled") {
		document.getElementById("chk-saveweapons").checked = false;
		return true;
	}
	
	const savedWeapons = JSON.parse(localStorage.getItem("splatoon2_elim_weapons"));
	const newweapons = [];
...
document.addEventListener("DOMContentLoaded", function() {
	...
	document.getElementById("chk-savesettings").addEventListener("change", function() {
		if (this.checked === false) {
			localStorage.setItem("splatoon2_elim_settings", "disabled");
			return;
		}
		saveSettings();
	});
	document.getElementById("chk-saveweapons").addEventListener("change", function() {
		if (this.checked === false) {
			localStorage.setItem("splatoon2_elim_weapons", "disabled");
			return;
		}
		saveWeapons();
	});

...
function saveLoadSettings(action) {
	if (action === "save" && allowsave === true) {
		...
		localStorage.setItem("splatoon_random", JSON.stringify(settings));
	}
	else if (action == "load") {
		if (localStorage.getItem("splatoon_random")) {
			var i = 0, k;
			var tempsettings = JSON.parse(localStorage.getItem("splatoon_random"));
		...
		localStorage.removeItem("splatoon_random");
	}
}
...
function imgurSettings() {
	...
	if (imgurparam.toLowerCase() === "disable") {
		...
		localStorage.removeItem("splatoon_random_imgur");
	}
	else if (imgurparam !== "") {
		//console.log("change + save imgur pass to: " + imgurparam);
		imgur.pass = imgurparam;
		imgur.enabled = true;
		localStorage.setItem("splatoon_random_imgur", JSON.stringify(imgur));
	}
	else if (localStorage.getItem("splatoon_random_imgur")) {
		//console.log("load previously saved data");
		imgur = JSON.parse(localStorage.getItem("splatoon_random_imgur"));
	}

...
srl.bingo = function (bingoList, size) {
	...
	if (LANG == '') { LANG = 'name'; }
		...
		if (document.getElementById("about_jp")) {
			document.getElementById("about_jp").classList.remove('hidden');
			document.getElementById("about_en").classList.add('hidden');
		}
	}
	var SEED = gup( 'seed' );
	if (SEED == "") {
	...
	srl.NewCard = function(mode) {
		let str = (mode ? "&mode="+mode : "");
		if (LANG !== 'name') { str += "&lang="+LANG; }
		if (PICK !== '') { str += "&pick="+PICK; }
		
		window.location = "?seed=" + (Math.floor(Math.random() * 999999) + 1) + str;
	}
	...
	var BINGO_URL = window.location.origin + window.location.pathname + "?seed=" + SEED;
	if (MODE !== '') { BINGO_URL += "&mode=" + MODE; }
	if (PICK !== '') { BINGO_URL += "&pick=" + PICK; }
	//add flags if the FIRST goal has a jp translation (it's assumed either all or no goals will have a translation)
	if (bingoList[1][0].jp !== undefined) {
		results.insertAdjacentHTML("beforeend", "<a href='" + BINGO_URL + "'><img src='" + window.location.origin + "/bingo/res/flags/United_States_of_America.png' alt='English' title='English' /></a> <a href='" + BINGO_URL + "&lang=jp" + "'><img src='" + window.location.origin + "/bingo/res/flags/Japan.png' alt='Japanese' title='Japanese' /></a>");
	}
	results.insertAdjacentHTML("beforeend", "<p>SRL Bingo <strong>v5</strong>&emsp;Seed: <strong>" + SEED + "</strong>&emsp;Card type: <strong>" + cardtype + "</strong></p>");
  ...
  if (size == 5) {
	...
	document.querySelectorAll("#bingo td:not(.popout)").forEach(td => td.addEventListener("click", function() {
		if (this.classList.contains("greensquare")) {
	...
	document.querySelectorAll(".popout").forEach(ele => ele.addEventListener("click", function() {
		const line = this.id;
	...
	document.querySelectorAll(".popout").forEach(ele => ele.addEventListener("mouseenter", function() {
		document.querySelectorAll("." + this.id).forEach(cell => cell.classList.add("hover"));
	}));
	document.querySelectorAll(".popout").forEach(ele => ele.addEventListener("mouseleave", function() {
		document.querySelectorAll("." + this.id).forEach(cell => cell.classList.remove("hover"));
	}));
...
function fullRowName(x) {
	if (x[0] == "d") {
		...
		if ((x[1] == 1) || (x[1] == 2) || (x[1] == 3) || (x[1] == 4) || (x[1] == 5)) {
			var a = "";
			switch (x[0]) {
				case "r":
					a = "row";
...
function rowpick(p) {
	if ((p == "row1") || (p == 1)) { return [1,2,3,4,5]; }
	else if ((p == "row2") || (p == 2)) { return [6,7,8,9,10]; }
	else if ((p == "row3") || (p == 3)) { return [11,12,13,14,15]; }
	else if ((p == "row4") || (p == 4)) { return [16,17,18,19,20]; }
	else if ((p == "row5") || (p == 5)) { return [21,22,23,24,25]; }
	else if ((p == "col1") || (p == 6)) { return [1,6,11,16,21]; }
	else if ((p == "col2") || (p == 7)) { return [2,7,12,17,22]; }
	else if ((p == "col3") || (p == 8)) { return [3,8,13,18,23]; }
	else if ((p == "col4") || (p == 9)) { return [4,9,14,19,24]; }
	else if ((p == "col5") || (p == 10)) { return [5,10,15,20,25]; }
	else if ((p == "tlbr") || (p == 11)) { return [1,7,13,19,25]; }
	else if ((p == "bltr") || (p == 12)) { return [5,9,13,17,21]; }
	else if ((p.length % 2 === 0) && (p.length > 1)) {
		p = p.match(/.{1,2}/g);
		p2 = [];
		for (i=0; i<p.length; i++) {
			if ((p[i][0] == "r") || (p[i][0] == "c")) {
				if ((p[i][1] == 1) || (p[i][1] == 2) || (p[i][1] == 3) || (p[i][1] == 4) || (p[i][1] == 5)) {
					for (x=0; x < rowpick(fullRowName(p[i])).length; x++) {
						p2.push(rowpick(fullRowName(p[i]))[x]);
					}
				}
			}
			else if (p[i][0] == "d") {
...
const rowArr = rowpick(PICK);
for (let nr of rowArr) {
	document.querySelectorAll("#slot"+nr).forEach(cell => cell.innerHTML = bingoBoard[nr].name);
}
}
} // setup
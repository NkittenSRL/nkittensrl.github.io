srl.bingo = function (bingoList, size) {
	...
	if (LANG == '') { LANG = 'name'; }
		...
		$("#about_jp").removeClass('hidden');
		$("#about_en").addClass('hidden');
	}
	var SEED = gup( 'seed' );
	if (SEED == "") {
	...
	if (PICK !== '') { BINGO_URL = BINGO_URL + "&pick=" + PICK; }
	//images from next line are broken offline, but work when uploaded
	results.append ("<a href='" + BINGO_URL + "'><img src='" + window.location.origin + "/bingo/res/flags/United_States_of_America.png' alt='English' title='English' /></a> <a href='" + BINGO_URL + "&lang=jp" + "'><img src='" + window.location.origin + "/bingo/res/flags/Japan.png' alt='Japanese' title='Japanese' /></a>");
  ...
  if (size == 5) {
    ...
    $('.popout').click(function() {
		...
		// changed window.open links below from /tools/bingo-popout.html to full url, to work on my site
        if (mode == 'simple-stream') {
          window.open('http://speedrunslive.com/tools/bingo-popout-basic.html#'+ name +'='+ items.join(';;;'),"_blank","toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=420, height=180"); }
        else {
		  ...
		  //window.open('http://speedrunslive.com/tools/bingo-popout.html#'+ name +'='+ items.join(';;;'),"_blank","toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=220, height=460"); }
    });
...
var rowArr = rowpick(PICK);
for (i=0; i<rowArr.length; i++) {
	$('#slot'+rowArr[i]).append(bingoBoard[rowArr[i]].name);
}
}
} // setup
...
function checkWeaponRequirements(wpn) {
	...
	const newestonly = document.getElementById("chk-newweaponsonly").checked;
	...
	if (newestonly === true && wpn.released !== weaponlist.updated.latestrelease) return false;
...
function weaponBox(wpn) {
	...
	txt += "<span class='subpic " + wpn.sub.toLowerCase().replaceAll(" ", "-") + "'></span><span class='weaponsub'>" + wpn.sub + "</span>";
	txt += "<span class='specialpic " + wpn.special.toLowerCase().replaceAll(/[ \.]/g, "-") + "'></span><span class='weaponspecial'>" + wpn.special + "</span>";
	txt += "<span class='level'><span class='levelcolor'>Level</span> <span class='levelnumber'>" + wpn.level + "</span></span>";
	txt += "<div class='DepletionContainer'><span class='depletionTxt' title='Special Cost'>Sp. Cost:</span> <span class='depletion'>" + ((wpn.specialcost === "" || wpn.specialcost === undefined) ? "Unknown" : wpn.specialcost + "p") + "</span></div>";
	txt += "</div>";
	return txt;
}
...
function clearSettings(sel) {
	...
	if (sel === "all") {
		document.getElementById("chk-newweaponsonly").checked = false;
		document.querySelectorAll("#costlevelsettings [id^='nr-min']").forEach(ele => {
			ele.value = ele.getAttribute("min");
		});
		document.querySelectorAll("#costlevelsettings [id^='nr-max']").forEach(ele => {
...
document.addEventListener("DOMContentLoaded", function () {
	...
	document.querySelector("label[for='chk-newweaponsonly']").innerHTML = `Newest Weapons Only${(weaponlist.updated.season !== undefined ? `<br>(${weaponlist.updated.season})` : "")}`;
	makeWeaponList();

	document.querySelectorAll("#ms-types, #ms-subs, #ms-specials, #chk-newweaponsonly").forEach(ele => ele.addEventListener("change", function () {
		filterWeaponList();
	}));

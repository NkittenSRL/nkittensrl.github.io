/* 
1 sep 2023:
fixed: when loading draftstatus, incorrect weapon images are shown in team boxes for any weapon that had weapons added before it in the weaponlist
(uses structuredClone() - supported in 90% of browsers, probably anyone using this site - didn't add it to splat1-2, as they shouldn't get any new weapons and adding it could only break things for no benefit)
*/
...
function startingFunc() {
	const multiban = [ //every array should contain scoped and unscoped chargers with the same kit (and the hero skin for regular splat charger)
		["Splat Charger", "Splatterscope", "Order Charger Replica"],
		["Z+F Splat Charger", "Z+F Splatterscope"],
		["Splat Charger CAM-O", "Splatterscope CAM-O"],
...
function getWeaponList(includeBanned = true, includeUnbanned = true, includeDrafted = true, allowReskin = "auto", allowScope = "auto", includeHidden = false) {
	const arr = [];
	const encountered = {};
	const reskinsVisible = document.getElementById("chk-showstoryreskins").checked;
	...
	for (let wpn of weapons) {
		...
		if (wpn.clones !== false){
			for (let clone of wpn.clones) {
				if (encountered[clone] === true && ((reskinsVisible === false && includeHidden === false) || allowReskin === false)) {
...
function getWeaponCount() {
	...
	for (let wpn of weapons) {
		...
		if (wpn.clones !== false) {
			for (let clone of wpn.clones) {
				if (counted[clone] === true) {
					category = "reskin";
					if (reskinsVisible === false) { elimstatus = "hidden"; }
					break; //if it finds a weapon that's alread counted in its clones list, you don't need to look at the rest of the clones list
				}
			}
		} else if (wpn.scopeclones !== undefined) {
			for (let scopeclone of wpn.scopeclones) {
...
function getWeaponClones(id, includeOwnId = false, forceReskin = false, forceScope = false) {
	...
	if (weapons[id].clones !== false && (document.getElementById("chk-mirrorbanreskin").checked === true || forceReskin === true)) {
		for (let i in weapons[id].clones) {
			const wpn = weaponmap[weapons[id].clones[i]];
...
function cleanDraftedWeapons(team = "all") {
	...
	document.querySelectorAll(selector).forEach(ele => {
		...
		if (["alpha","bravo"].includes(TEAM)) { //ONLY do this stuff if TEAM is "alpha" or "bravo"; hopefully fixes a bug - when unchecking #chk-mirrorbanscope, if you had drafted Splatterscope and #chk-mirrorbanreskin was also checked, it would first clear the drafted status from Splat Charger, including its reskin (Order Charger Replica), and then it would try to do it again with the Order Charger Replica (because querySelectorAll returns a NON-live NodeList, it didn't remove that element from the list), but now TEAM will be "available" which causes an error when checking indexOf later
...
function saveWeapons() {
	...
	const tempDraft = structuredClone(draftstatus);
...
function loadWeapons() {
	...
	savedWeapons.draft.bravo = savedWeapons.draft.bravo.map(savedWpn => weapons.findIndex(wpn => wpn.name === savedWpn));
...
document.addEventListener("DOMContentLoaded", function() {
	...
	document.getElementById("chk-showstoryreskins").addEventListener("change", function() {
		const cList = document.body.classList;
	...
	document.querySelectorAll("#chk-enabledraft, #chk-showbanhistory, #chk-showstoryreskins, #dd-displaymode").forEach(ele => ele.dispatchEvent(new Event("change")));
